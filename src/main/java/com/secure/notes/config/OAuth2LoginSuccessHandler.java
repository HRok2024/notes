...
public class OAuth2LoginSuccessHandler extends SavedRequestAwareAuthenticationSuccessHandler {
    ...
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication) throws ServletException, IOException {
        ...
        if ("github".equals(oAuth2AuthenticationToken.getAuthorizedClientRegistrationId()) || "google".equals(oAuth2AuthenticationToken.getAuthorizedClientRegistrationId())) {
            ...
            System.out.println("어써"+attributes);
            ...
            System.out.println("HELLO OAUTH: " + email + " : " + name + " : " + username);
            //이메일로 유저를 찾는 서비스 필요 (가입되어 있는지 확인)
            userService.findByEmail(email)
                    .ifPresentOrElse(user -> {
                        ...
                        if (userRole.isPresent()) {
                            newUser.setRole(userRole.get()); //DB에 유저 권한 튜플이 있다면 새로운 유저 객체에 set
                        } else {
                            //DB에 유저 권한이 없을경우
                        ...
                        userService.registerUser(newUser); //신규유저 DB에 저장 (가입을 oauth로 함)
                        DefaultOAuth2User oauthUser = new DefaultOAuth2User(
        ...
        //attributes 에서 필요한 정보를 가져와서 이메일을 로그로 출력
        String email = (String) attributes.get("email");
        System.out.println("OAuth2LoginSuccessHandler: " + username + " : " + email);
        //유저의 권한은 이메일로 유저를 DB에서 찾아 권한을 가져온다
